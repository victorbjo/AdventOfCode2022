...
class Monkey:
    ...
    def run(self, monkeys, common):
        for idx, item in enumerate(self.startingItems):
            self.monkeyBusiness += 1
            self.startingItems[idx] = self.performOperation(item)

            #self.startingItems[idx] = int(self.startingItems[idx])
            if self.startingItems[idx]%self.test == 0:
                monkeys[self.trueCase].startingItems.append(self.startingItems[idx]%common)
            else:
                monkeys[self.falseCase].startingItems.append(self.startingItems[idx]%common)
        self.startingItems = [] 
...
for monkey in monkeys:
    common *= int(monkey.test)
for x in range(10000):
    for idx, monkey in enumerate(monkeys):
        monkey.run(monkeys, common)
