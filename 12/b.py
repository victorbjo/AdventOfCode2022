...
def djikstra(queue, nodeList, data):
    queue = [queue]
    queue[0].cheapest = 0
    ...
    while queue:
        ...
        if current.val == "E":
            cheapest = min(cheapest, current.cheapest)
        if current.x > 0:
...
for idx, line in enumerate(data):
    for idx2, letter in enumerate(line):
        if letter == "a":
            A.append(nodeList[idx][idx2])
#print(A)
print(nodeList[0][2].cheapest)
while A:
    shortest = min(shortest, djikstra(A.pop(0), nodeList, data))
print(nodeList[0][2].cheapest)
print(shortest)